...
exports.register = async (req, res) => {
  ...
  try {
    ...
    if (req.files.length > 0) {
      req.files.map(file => {
        newUser.receipt = "http://192.168.93.26:8888/public/" + file.originalname
      })
    } else {
      newUser.receipt = "http://192.168.93.26:8888/public/default.jpg"
    }
...
exports.GetAllUser = async (req,res)=>{
  try {
    const result = await User.find().populate("publication").populate("classes").sort({createdAt:-1})
...
exports.UpdateUser= async (req, res) => {
  try {
    ...
    if (req.files.length > 0) {
      req.files.map(file => {
        user.receipt = "http://192.168.93.26:8888/public/" + file.originalname
      })
    }

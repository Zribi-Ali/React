...
exports.GetAllClasse = async (req, res) => {
      try {
            const result = await classe.find().populate("formation").populate({
              path: "cours",
              populate: {
                path: "user",
              },
            }).populate("etudiant").populate("proffeseur").sort({createdAt:-1});
...
exports.GetOneClasse = async (req, res) => {
      try {
            const result = await classe.findOne({ _id: req.params.id }).populate("formation").populate({
              path: "cours",
              populate: {
                path: "user",
              },
            }).populate("etudiant").populate("proffeseur").populate("cours");
            res.send({ response: result, message: "avoir classe avec succès" });
      } catch (error) {
